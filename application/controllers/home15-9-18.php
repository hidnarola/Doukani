    ...
    public function index($slug = NULL, $load_data = NULL) {
        ...
        if ($_SERVER['HTTP_HOST'] == $full_url) {
            ...
            if (($slug == NULL || in_array(strtolower($seg1), array('abudhabi', 'ajman', 'dubai', 'fujairah', 'ras-al-khaimah', 'sharjah', 'umm-al-quwain'))) && ($seg2 == NULL || $seg2 == "followers")) {
                ...
                $return_res = $this->db->query('(select "cat_slug" AS val from category where category_slug = "' . $slug . '" ) UNION (select "sub_cat_slug" AS val from sub_category where sub_category_slug = "' . $slug . '" ) UNION (select "selleruser_slug" AS val from user where user_slug = "' . $slug . '") UNION  (select "prod_slug" AS val from product where product_slug = "' . $slug . '")');
                $return_data = $return_res->row();
    ...
    public function advanced_search() {
        ...
        if (isset($_REQUEST['default']) || isset($_REQUEST['vehicle_submit']) || isset($_REQUEST['real_estate_submit']) || isset($_REQUEST['shared_submit']) || isset($_REQUEST['car_number_submit']) || isset($_REQUEST['mobile_number_submit'])) {
            ...
            if (isset($current_user) && !empty($current_user)) {
                $like_fav_sql = ' left join like_product lp on  p.product_id left join favourite_product fp on fp.product_id = p.product_id ';
		...
		if(cmn.number_for="car_number" and cmn.repeating_number="-1","More than 5",if(cmn.number_for="mobile_number" and cmn.repeating_number="-1","More than 8",rn.rep_number)) as car_repeating_number,mo.operator_name as mobile_operator,cmn.mobile_number ';
            ...
            $query = "SELECT p.product_id,p.category_id,p.product_posted_by,p.product_name,p.product_image,p.product_price,p.product_status,p.product_is_inappropriate ,p.product_total_views, p.product_total_favorite,if(u.nick_name!='',u.nick_name,u.username) as username1,c.catagory_name, u.username, u.profile_picture,u.facebook_id,u.twitter_id,u.google_id,p.product_is_sold,p.product_total_likes ,p.product_slug,u.user_slug,
              (IF(p.product_image IS NULL OR p.product_image='',0,1) +
               ...
               IF(count_image IS NULL  OR count_image='',0,count_image)) as MyTotal,
...
//            $data['seo']['keyword'] = $data['keyword_'] = str_replace(' ', ', ', $seo_keyword) . ', doukani, classified, lowest, latest';
    ...
    public function categories() {
        ...
        $wh_category_data = array('FIND_IN_SET(0, sub_category_type) > 0');
...
';
    ...
    public function load_more_storelisting() {
        ...
        if (isset($filter_val) && isset($user_id)) {
            ...
            if (isset($_POST['product_view']) && $_POST['product_view'] == 'list') {
                if ($cat_id == 7)
                    $products = $this->dbcommon->get_vehicle_products(NULL, NULL, NULL, 15, $start, $store_det->store_owner, $search);
                elseif ($cat_id == 8)
                    $products = $this->dbcommon->get_real_estate_products(NULL, NULL, NULL, 15, $start, $store_det->store_owner, $search);
                else
                    $products = $this->dbcommon->get_product_by_categories(NULL, NULL, NULL, 15, $start, NULL, $store_det->store_owner, $search);
            } else
    ...
    function get_store_products() {
        ...
        if (isset($user_id)) {
            ...
            if (isset($_POST['product_view']) && $_POST['product_view'] == 'list') {
                if ($cat_id == 7)
                    $products = $this->dbcommon->get_vehicle_products(NULL, NULL, NULL, 15, NULL, $store_data->store_owner, $search);
                elseif ($cat_id == 8)
                    $products = $this->dbcommon->get_real_estate_products(NULL, NULL, NULL, 15, NULL, $store_data->store_owner, $search);
                else
                    $products = $this->dbcommon->get_product_by_categories(NULL, NULL, NULL, 15, NULL, NULL, $store_data->store_owner, $search);
            } else
                $products = $this->dbcommon->get_my_listing($user_id, $start, 15, $search, $user_status, $store_user[0]->user_role, 'store');
...
//            if (send_mail('rep@narola.email', 'Report For A Doukani Product', $new_data)) {
    ...
    function load_more_subcategories() {
        ...
        $query = "category_id= '" . $category_id . "' order by sub_cat_order ASC LIMIT 10, 1000000000";
        $subcat = $this->dbcommon->filter('sub_category', $query);
