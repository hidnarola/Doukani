...
class Classifieds extends CI_Controller {
    ...
    public function categories_delete($cat_id = null) {
		$num_row = $this->dbcommon->getnumofdetails_("* from product where category_id = " . $cat_id . "");
        if ($num_row < 1) {
			$target_dir = document_root . category;
			$data = array();
			$where = " where category_id='" . $cat_id . "'";
			$sub_category = $this->dbcommon->getdetails('sub_category', $where);
			...
			if(empty($sub_category)):
				...
				$this->session->set_flashdata(array('msg' => 'Sub-category having this Category, You can not delete this Category ', 'class' => 'alert-info'));
				redirect('admin/classifieds/categories');
			endif;	
		}
		else
		{
			$this->session->set_flashdata(array('msg' => 'Sorry, you can not delete this category.Product having this Category', 'class' => 'alert-info'));
			redirect('admin/classifieds/categories');
		}
    }
...
//    Sub Categories Functions
    ...
    public function subCategories_delete($cat_id = null) {
        ...
        $where = " where sub_category_id='" . $cat_id . "'";
        $category = $this->dbcommon->getdetails('sub_category', $where);		
		$num_row = $this->dbcommon->getnumofdetails_("* from product where sub_category_id = " . $cat_id . "");
		...
		{
			$this->session->set_flashdata(array('msg' => 'Sorry, You can not delete this sub-category. Product having this sub-category.', 'class' => 'alert-info'));
            redirect('admin/classifieds/subCategories/' . $category[0]->category_id);
		}
    }
...
//    Listing function
    public function listings() {
        ...
        if ($user->user_role == 'generalUser') {
			$query = "SELECT p.product_id,p.product_name,p.product_image,p.product_price,p.product_status,p.product_is_inappropriate , 
                ...
                p.product_is_inappropriate != 'Inappropriate' and p.product_posted_by = " . $user->user_id. " AND p.product_deactivate is null";
        } else {		
            $query = "SELECT p.product_id,p.product_name,p.product_image,p.product_price,p.product_status,p.product_is_inappropriate , 
                c.catagory_name 
				FROM product as p 
				left join category as c on c.category_id=p.category_id
				where p.is_delete = 0 and p.category_id=c.category_id and p.product_is_inappropriate != 'Inappropriate' ". " AND p.product_deactivate is null";
				// 
        }
    ...
    public function listings_add() {
		...
		$user1  = $this->dbcommon->getrowdetails('user', $where);
        ...
        if (!empty($_POST)):
            ...
            if (isset($result)):
                ...
                $ads_left 			= (int) $user1->userAdsLeft;
    ...
    public function listings_edit($pro_id = null) {
        ...
        if($product_type=='vehicle')
            ...
            $product = $this->dbcommon->get_real_estate_products_admin(null,null,$pro_id,null,null);
				...
				if(isset($_POST['default_submit'])) 
				{
					...
					if ($this->form_validation->run() == FALSE):
						...
						$data = array(
							...
							'product_price' => str_replace(",","",$_POST['pro_price']),
							'admin_modified_at' => date('y-m-d H:i:s'),                     
							'product_brand' => 0,
							'state_id' => $_POST['state'],
        ...
        if ($filter_val == '0') {
            ...
            if ($filter_opt == '0') {
				...
				$query	=	$this->db->query("select * from product p
							right join 	featureads f on f.product_id=p.product_id
							where p.is_featured=0 and  p.is_delete = 0 and p.product_is_inappropriate != 'Inappropriate' ".$wh." group by p.product_id");
				
				$main_data['product'] = $query->result_array();		
		} elseif($filter_val == 'unfeatured') { 					
				$query	=	$this->db->query("select * from product p
							right join 	featureads f on f.product_id=p.product_id
							where p.is_featured=1 and  p.is_delete = 0 and p.product_is_inappropriate != 'Inappropriate' group by p.product_id order by f.id desc");
	...
	{		
		...
		{
			$res	=	$this->db->query('select category_id,sub_category_id,product_id from product where product_is_inappropriate="Approve" and is_delete=0 and product_deactivate is null and product_id='.(int)$val. '');
			...
			if($res->num_rows()>0)
			{
				...
				$this->db->query("update product set is_featured=0 where   product_id=".(int)$data[0]['product_id']);		
			}			
		}		
		$this->session->set_flashdata(array('msg' => 'product(s) featured successfully', 'class' => 'alert-info'));
        redirect('admin/classifieds/listings');
		
	}

	function update_unfeatured()
	{		
		$checked_values	=	 explode(",",$_POST['checked_val']);		
		foreach($checked_values as $val)
		{
			$res	=	$this->db->query('select product.category_id,product.sub_category_id,product.product_id from product 
						left join featureads f on f.product_id=product.product_id
						where product.is_delete=0 and product.is_featured=0 and product.product_id='.(int)$val. '');
			...
			if($res->num_rows()>0)
			{
				...
				$result = $this->dbcommon->update('product',$arra,$up);
			}			
		}		
		$this->session->set_flashdata(array('msg' => 'product(s) un-featured successfully', 'class' => 'alert-info'));
        redirect('admin/classifieds/listings');
	}
	...
	function show_emirates_postadd() {
        //$value = 4;
        $query 					= "country_id= " . $_POST['value'];        		
		$main_data['state'] 	= $this->dbcommon->filter('state', $query);
		$main_data['sel_city'] 	= $_POST['sel_city'];
		echo $this->load->view('listings/show_state_postad', $main_data, TRUE);
        exit;
    }
