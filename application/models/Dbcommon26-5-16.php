...
class Dbcommon extends CI_Model {
    ...
    function get_products(){
        ...
        ,user.facebook_id,user.twitter_id,user.google_id,product.product_slug',FALSE);
    ...
    function get_product_slugwise($product_slug=null){
    ...
    function get_product_by_categories($cat=null, $subcat=null, $current_pro_id=null, $limit=null, $start=null,$rand=null,$posted_by=null){
    ...
    function get_my_listing($user_id,$start,$limit,$search = NULL){
        ...
        if($search!=NULL) {            
            if($search=='new')
                $this->db->where('product.admin_modified_at >= DATE(NOW()) - INTERVAL 7 DAY');
	...
	function get_my_listing_count($user_id,$search = NULL){
	...
	function get_my_seller_listing($user_id,$start,$limit){
        ...
        $this->db->select('product.product_id,product.product_posted_by,product.product_posted_time,product.product_name,product.product_image,product.product_price,product.product_status,product.product_is_inappropriate,product.product_total_views,product.product_total_favorite,category.category_id,category.catagory_name,user.username, user.profile_picture, product_realestate_extras.Emirates, product_realestate_extras.PropertyType, product_realestate_extras.Bathrooms, product_realestate_extras.Bedrooms, product_realestate_extras.Area, product_realestate_extras.Amenities, product_realestate_extras.neighbourhood, product_realestate_extras.address, product_realestate_extras.furnished, product_realestate_extras.pets, product_realestate_extras.broker_fee, product_realestate_extras.free_status, product_realestate_extras.ad_language, product_vehicles_extras.model, product_vehicles_extras.millage, product_vehicles_extras.color, product_vehicles_extras.type_of_car, product_vehicles_extras.year, product_vehicles_extras.make, product_vehicles_extras.vehicle_condition,if(user.nick_name!="",user.nick_name,user.username) as username1,user.nick_name,product.product_is_sold,mileage.name mileagekm,color.name colorname,user.facebook_id,user.twitter_id,user.google_id,product.product_total_likes`,product.product_total_likes,product.product_slug');
        $this->db->from('product');
        $this->db->join('category', 'category.category_id = product.category_id');
        $this->db->join('user', 'user.user_id = product.product_posted_by');
        $this->db->join('product_realestate_extras', 'product_realestate_extras.product_id = product.product_id','left');
	...
	function getBanner_array($type,$display,$option,$cat_id,$sub_cat_id,$not_this=NULL){	
		...
		SELECT ss.* FROM
                  (SELECT
                    case when c.bidding_option = 'duration'  then (bcd.count <= c.impression_day and bcd.date = '2016-03-11' and '2016-03-11'
                    between c.expiry_start_date and c.expiry_end_date or is_endate='1')
                        when c.bidding_option = 'cpm' then c.impression_count <= c.impression_day
                        when c.bidding_option = 'cpc' then c.click_count <= c.clicks_day end as data,
                        c.*,bcd.* FROM custom_banner c left join banner_cnt_duration bcd on bcd.banner_id=c.ban_id
                        WHERE c.status = 1 AND c.ban_show_status IN ('ios','both') AND c.ban_type_name = 'feature' AND c.pause_banner = 'no') ss
                    where ss.data=1 order by RAND() LIMIT 5
		...
		*/
        ...
        if($not_this!=NULL)
            $this->db->where_not_in('ban_id',array($not_this));
    ...
    function getBanner_forCategory($type,$display,$cat_id,$sub_cat_id,$not_this= NULL){
        //echo 'Not this========'.$not_this.'=========';
        // echo '<br>';
        // echo $type.','.$display.','.$cat_id.','.$sub_cat_id.','.$not_this;
        // echo '<br>';
        $data_status = 0;
        $mydata = $this->getBanner_array($type,$display,'here',$cat_id,$sub_cat_id,$not_this);
        if($mydata  == 1)            
            $data_status=1;
        else
            $mydata = $this->getBanner_array($type,$display,'here',$cat_id,$sub_cat_id,$not_this);
        ...
        if(sizeof($mydata)>0 && $mydata[0]['ban_id']!='') {
            ...
            elseif(isset($option) && $option=='duration') {             
                ...
                // (((CURDATE() between expiry_start_date and expiry_end_date) and expiry_end_date<>"0000-00-00") or (expiry_start_date<=CURDATE() and is_endate=1))))');
            ...
            // print_r($data);
            // echo "</pre>";
            if(sizeof($data)==0 && $data_status==0) {                               
                $not_this = $mydata[0]['ban_id'];
                try {                    
                    return $this->getBanner_forCategory($type,$display,'here',$cat_id,$sub_cat_id,$not_this);                
                }
                catch(Exception $e) {                    
                    return $this->getBanner_forCategory($type,$display,'here',$cat_id,$sub_cat_id,$not_this); 
                }
    ...
    function get_featured_ads($start,$limit){  
        $this->db->select('featureads.product_id,featureads.User_Id,featureads.dateFeatured,featureads.dateExpire,featureads.cat_id,featureads.subcat_id,if(user.nick_name!="",user.nick_name,user.username) as username1,product.product_name,product.product_posted_by,product.product_total_likes,product.product_price, user.*, category.*,product.product_image,product.product_is_sold,product.product_slug',FALSE);
		...
		$bet	=	'CURDATE() between DATE_FORMAT(featureads.dateFeatured,"%Y-%m-%d") and DATE_FORMAT(featureads.dateExpire,"%Y-%m-%d")';
		$this->db->where($bet);
        $this->db->where('product.is_delete' ,0);
	...
	function get_today_product($sub_cat)
	{
        ...
        $this->db->from('product');
		$this->db->where('sub_category_id' ,$sub_cat);
        $this->db->where('date(product_posted_time)' ,date('Y-m-d'));
	...
	public function  users_ds_update() {
		...
		if(date('Y-m-d')==date('Y-m-01')) {
            ...
            ///for general User
		    $query		=	' id=3 limit 1';
    		$ads_cnt_genuser	=	$this->filter('settings', $query);
			
			if($ads_cnt_genuser[0]['val']>0)
				$cnt_ads_genuser	=	$ads_cnt_genuser[0]['val'];
			else
				$cnt_ads_genuser	=	15;
			
			///for store User
            $query      =   ' id=8 limit 1';
            $ads_cnt_storeusr    =   $this->filter('settings', $query);
            
            if($ads_cnt_storeusr[0]['val']>0)
                $cnt_ads_storeusr    =   $ads_cnt_storeusr[0]['val'];
            else
                $cnt_ads_storeusr    =   10;

			$chk_user	=	$this->db->query('select user_id,from_date,to_date,userAdsLeft,userTotalAds from user where is_delete=0 and status="active"');
				...
				foreach($res_user as $r)
				{
					...
					$wh_user	=	array(
                                ...
                                if($r['user_role']=='generalUser')
                                    $cnt_ads    =    $cnt_ads_genuser;
                                elseif($r['user_role']=='storeUser')
                                    $cnt_ads    =    $cnt_ads_storeusr;
                                else
                                    $cnt_ads    =   15;
	...
	public function crop_product_image($src, $width, $height,$destination,$folder,$file_name) {
        ...
        if (in_array(strtolower($type), $allowed_type)) {
			...
			if($folder=='product_detail' && $w <= image_product_detail_width && $h<=image_product_detail_height) {
			...
			if(file_exists(document_root.product.'medium/'.$file_name))							
				$this->watermarkImage ($file_name, $WaterMark, $dest, 50,'medium');
			...
			if(file_exists(document_root.product.'small/'.$file_name))
				$this->watermarkImage($file_name, $WaterMark, $dest, 50,'small');				
			
		/*	$WaterMark  = site_url().'assets/front/images/big_watermark.gif';							 								 
	...
	public function watermarkImage($SourceFile, $WaterMark, $DestinationFile=NULL, $opacity,$folder) { 
		//$dest	=	site_url().product.'original/'.
		  if(!empty($SourceFile)) {
            ...
            // imagecopymerge($image, $watermark, imagesx($image) - $sx - $marge_right, imagesy($image) - $sy - $marge_bottom, 0, 0, imagesx($watermark), imagesy($watermark), 50);
           }
    		 if ($DestinationFile<>'') {
    ...
    ***/        
    function generate_slug($string = NULL,$for = NULL) {
        ...
        if($for=='P')
            ...
            $for1 = $for.'_';
        $string = $for1.preg_replace('/[^A-Za-z0-9\-]/', '', $string).'-'.time(); // Removes special chars.
