...
(function($, window, document) {
	...
	var EmojiArea_Plain = function($p, id, options) {
	...
	EmojiArea_Plain.prototype.insert = function(emoji) {
		if (!$.emojiarea.icons.hasOwnProperty(emoji))
			return;
		util.insertAtCursor(emoji, this.$p[0]);
		/*
		 * MODIFICATION: Following line was added by Igor Zhukov, in order to
		 * save recent emojis
		 */
		util.emojiInserted(emoji, this.menu);
		this.$p.trigger('change');
	};
	...
	var EmojiArea_WYSIWYG = function($p, id, options) {
		var self = this;

		this.options = options || {};
		if ($($p).attr('data-emoji-input') === 'unicode')
    ...
    this.emojiPopup = options.emojiPopup;
		this.$editor = $('<div>').addClass('emoji-wysiwyg-editor').addClass($($p)[0].className);
    this.$editor.data('self', this);

    if ($p.attr('maxlength')) {
      this.$editor.attr('maxlength', $p.attr('maxlength'));
    }
	
    var unicodeToImageText = this.emojiPopup.unicodeToImage($p.val());
    ...
    $p.after("<i class='emoji-picker-icon emoji-picker " + this.options.popupButtonClasses + "' data-id='" + id + "' data-type='picker'></i>");

		$p.hide().after(this.$editor);
		this.setup();
  ...
  EmojiArea_WYSIWYG.prototype.updateBodyPadding = function(target) {
	...
	EmojiArea_WYSIWYG.prototype.onChange = function(e) {
		this.$p.val(this.val()).trigger('change');
	};
