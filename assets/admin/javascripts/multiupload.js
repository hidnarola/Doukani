function multiUploader(config){
	...
	multiUploader.prototype._init = function(){
		if (window.File && 
			...
			window.FileList && 
			window.Blob) {		
			 var inputId = $("#"+this.config.form).find("input[name='multiUpload']").eq(0).attr("id");
		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="'+base_url+'assets/images/image.png" />';
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
						sampleIcon = '<img src="'+base_url+'assets/images/unknown.png" />';
						errorClass =" invalid";
					} 
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="'+base_url+'assets/images/ajax-loader.gif" /></div></div>';
				}
			}
